...
class ActivateReferralCodeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if get_user_model().objects.filter(phone=attrs['phone']).first().referral_code.code == attrs['referral_code']:
            raise serializers.ValidationError('Нельзя активировать свой реферальный код')
